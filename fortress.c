...
static int fortressHasIntersection(Pos3 bmin0, Pos3 bmax0,
        Pos3 bmin1, Pos3 bmax1)
{
...
static uint64_t fortressChunkGenerateRnd(uint64_t worldSeed, int chunkX, int chunkZ)
{
    uint64_t rnd;
...
static void setFortressAttemptSeed(uint64_t *s, int cx, int cz)
{
...
const FortressPieceInfo fortress_info[] = {
...
Piece *addFortressPiece(FortressPieceEnv *env, int typ, int x, int y, int z, int depth, int facing, int pending)
{
    ...
    {
        Piece *q = env->list + i;
        if (fortressHasIntersection(q->bb0, q->bb1, p->bb0, p->bb1)) {
...
int getFortressPieces(Piece *list, int n, int mc, uint64_t seed, int chunkX, int chunkZ)
{
    ...
    if (mc <= MC_1_15)
    {
        setFortressAttemptSeed(&rng, chunkX, chunkZ);
        nextInt(&rng, 3);
        nextInt(&rng, 8);
        nextInt(&rng, 8);
    }
    ...
    {
        rng = fortressChunkGenerateRnd(seed, chunkX, chunkZ);
    }
