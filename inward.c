...
static int g_inwardness_max_x = 2;  // 1..10; default 2x at the four-region intersection
...
// Objective geometry:
//   * The four regions meet at block coordinate
//       ((corner_rx + 1) * 256, (corner_rz + 1) * 256).
//   * Piece area is its horizontal XZ bounding-box area, not volume and not a
//     unit count per piece.
//   * A piece touching the four-region intersection gets g_inwardness_max_x area weight.
//   * A piece at the maximum horizontal extent allowed by fortress.c gets 1.0x.
//
// The 310-block maximum comes directly from fortress.c: the outermost pinned
// start can be 190 blocks from the four-region boundary, extendFortress() allows
// an anchor to move at most 112 blocks from the start, and the largest outward
// horizontal bbox extension is 18 blocks. We use Chebyshev distance because the
// piece only needs to be close in both X and Z to be considered inward.
#define OBJECTIVE_WEIGHT_SCALE 1000ULL
#define OBJECTIVE_MIN_WEIGHT   OBJECTIVE_WEIGHT_SCALE
#define OBJECTIVE_MAX_DISTANCE 310ULL

static inline uint64_t piece_footprint_area(const Piece *p)
{
    uint64_t w = (uint64_t)(p->bb1.x - p->bb0.x);
    uint64_t d = (uint64_t)(p->bb1.z - p->bb0.z);
    return w * d;
}

static inline uint64_t axis_distance_to_point(int p, int lo, int hi)
{
    if (p < lo)
        return (uint64_t)(lo - p);
    if (p > hi)
        return (uint64_t)(p - hi);
    return 0;
}

static inline uint64_t piece_inward_weight(const Piece *p, int ix, int iz)
{
    uint64_t dx = axis_distance_to_point(ix, p->bb0.x, p->bb1.x);
    uint64_t dz = axis_distance_to_point(iz, p->bb0.z, p->bb1.z);
    uint64_t dist = dx > dz ? dx : dz;
    uint64_t max_weight = (uint64_t)g_inwardness_max_x * OBJECTIVE_WEIGHT_SCALE;
    if (dist >= OBJECTIVE_MAX_DISTANCE)
        return OBJECTIVE_MIN_WEIGHT;

    return max_weight -
           (max_weight - OBJECTIVE_MIN_WEIGHT) * dist / OBJECTIVE_MAX_DISTANCE;
}

static inline uint64_t piece_objective_score(const Piece *p, int ix, int iz)
{
    return piece_footprint_area(p) * piece_inward_weight(p, ix, iz);
}
...
static int summarize_candidate(uint64_t base_feed, CandidateSummary *out)
{
    ...
    out->total_pieces = 0;
    out->objective_score = 0;

    const int intersection_x = (g_corner_rx + 1) * 256;
    const int intersection_z = (g_corner_rz + 1) * 256;
    ...
    for (int d = 0; d < REGION_COUNT; ++d) {
        ...
        for (int i = 0; i < n; ++i)
            out->objective_score +=
                piece_objective_score(&list[i], intersection_x, intersection_z);
    }
...
static void make_detailed_result(const CandidateSummary *sum, SearchResult *out)
{
    ...
    out->objective_score = sum->objective_score;
...
// output code are search plumbing. If the goal changes later, change this
// comparator (and, only if needed, the ObjectiveKey construction).
//
// Current objective:
//   1. Maximize the sum of (horizontal piece area * inwardness weight) over
//      all four fortresses.  The weight is 2x at the four-region intersection
//      and approaches 1x at the maximum fortress extent.
//   2. Smaller world seed as a deterministic tie-breaker.
//   3. Smaller corner coordinates as a final deterministic tie-breaker.
static int objective_compare(const ObjectiveKey *a, const ObjectiveKey *b)
{
    if (a->objective_score != b->objective_score)
        return a->objective_score > b->objective_score ? 1 : -1;
    if (a->world_seed != b->world_seed)
        return a->world_seed < b->world_seed ? 1 : -1;
    if (a->corner_rx != b->corner_rx)
        return a->corner_rx < b->corner_rx ? 1 : -1;
    if (a->corner_rz != b->corner_rz)
        return a->corner_rz < b->corner_rz ? 1 : -1;
    return 0;
}
...
{
    ObjectiveKey k;
    k.objective_score = s->objective_score;
    k.world_seed = s->world_seed;
    k.corner_rx = g_corner_rx;
    k.corner_rz = g_corner_rz;
    return k;
}
...
{
    ObjectiveKey k;
    k.objective_score = r->objective_score;
    k.world_seed = r->world_seed;
    k.corner_rx = r->corner_rx;
    k.corner_rz = r->corner_rz;
    return k;
}
...
static void print_usage(const char *argv0)
{
    ...
    printf("  --inwardness N      Intersection weight [1..10] for the objective score.\n");
    printf("                     Default 2 means a piece at the 4-region intersection is\n");
    printf("                     worth 2.0x its XZ area; 10 means 10.0x.\n");
    printf("  Objective ranking is isolated in objective_compare(); current score is XZ piece area weighted by inwardness.\n");
...
static void print_result(const SearchResult *r, int rank)
{
    ...
    printf("#%d worldSeed=0x%012" PRIx64
        ...
        r->total_pieces);
    printf("    objectiveScore=%" PRIu64 " (area * inwardness; 1000=1.0x, max=%d000=%d.0x at intersection)\n",
           r->objective_score, g_inwardness_max_x, g_inwardness_max_x);
}
...
int main(int argc, char **argv)
{
    ...
    if (argc == 3 && argv[1][0] != '-') {
        ...
        for (int i = 1; i < argc; ++i) {
            if (!strcmp(argv[i], "-h") || !strcmp(argv[i], "--help")) {
                ...
                legacy_pin_used = 1;
            } else if (!strcmp(argv[i], "--inwardness")) {
                int inwardness;
                if (++i >= argc || !parse_int_arg(argv[i], &inwardness) ||
                    inwardness < 1 || inwardness > 10)
                    die("--inwardness must be an integer in [1,10]");
                g_inwardness_max_x = inwardness;
    ...
    printf("Objective: XZ area weighted from %.1fx at the 4-region intersection to 1.0x outward\n",
           (double)g_inwardness_max_x);
